...
int map_block(u_int blockno) {
	...
	return syscall_mem_alloc(0, diskaddr(blockno), PTE_D); //使用系统调用分配内存，当前进程是env；将blockno转成对应虚拟地址
...
int dir_lookup(struct File *dir, char *name, struct File **file) {
	...
	nblock = dir->f_size / BY2BLK;       //块数 = 文件总大小 ÷ 每块大小
	...
	for (int i = 0; i < nblock; i++) {
		...
		try(file_get_block(dir, i, &blk));       //将某个指定文件指向的磁盘块读入内存
...
int walk_path_at(struct File *par_dir, char *path, struct File **pdir, struct File **pfile, char *lastelem) {
	...
	file = par_dir;
	dir = par_dir;
//	strcpy((char*)name, (char*)dir->f_name);
	name[0] = 0;
	if (pdir) {
		*pdir = dir;
	}
